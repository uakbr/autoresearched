#!/usr/bin/env python3
"""
Baseline comparison for Emotional-Learning autoresearch v2.

Runs six baselines on the fixed test set and prints a comparison table.
...
# ============================================================
# EXTRA_TRAIN — duplicated from train.py to avoid import
# side effects (train.py mutates global dataset state on import)
# ============================================================
...
def random_baseline(test_texts):
    """Predict a random label for each text (deterministic with seed)."""
    rng = random.Random(42)
    return [rng.choice(CLASSES) for _ in test_texts]


def majority_baseline(test_texts, train_labels):
    """Always predict the most common label in training data."""
    counts = Counter(train_labels)
    majority_label = counts.most_common(1)[0][0]
    return [majority_label] * len(test_texts)


def rule_based_baseline(test_texts):
    """Use MoodAnalyzer().predict_label() directly (no ML)."""
    analyzer = MoodAnalyzer()
    preds = []
    for t in test_texts:
        with contextlib.redirect_stdout(io.StringIO()):
            preds.append(analyzer.predict_label(t))
    return preds


def tfidf_logreg_baseline(train_texts, train_labels, test_texts):
    """TF-IDF + LogisticRegression, no custom features."""
...
def tfidf_svc_baseline(train_texts, train_labels, test_texts):
    """TF-IDF + LinearSVC, no custom features."""
    vec = TfidfVectorizer(max_features=300, stop_words="english",
                          ngram_range=(1, 2))
    X_train = vec.fit_transform(train_texts)
    X_test = vec.transform(test_texts)
    clf = LinearSVC(max_iter=5000, random_state=42, C=5.0)
    clf.fit(X_train, train_labels)
    return clf.predict(X_test).tolist()


def our_cascade(train_texts, train_labels, test_texts):
    """Full current system: two-stage cascade + 15 custom features + binary CountVectorizer."""
    # Vectorizer
    ...
    X_bow_train = vectorizer.fit_transform(train_texts)
...
if __name__ == "__main__":
    # Apply word-list expansions so rule-based and cascade match train.py
    ...
    # Define baselines
    baselines = [
        ("Random",          lambda: random_baseline(test_texts)),
        ("Majority",        lambda: majority_baseline(test_texts, train_labels)),
        ("Rule-based",      lambda: rule_based_baseline(test_texts)),
        ("TF-IDF+LogReg",   lambda: tfidf_logreg_baseline(train_texts, train_labels, test_texts)),
        ("TF-IDF+SVC",      lambda: tfidf_svc_baseline(train_texts, train_labels, test_texts)),
        ("Our Cascade",     lambda: our_cascade(train_texts, train_labels, test_texts)),
    ]
    ...
    header = f"{'Baseline':<20} {'Macro-F1':>10} {'Accuracy':>10} {'Pos-F1':>8} {'Neg-F1':>8} {'Neu-F1':>8} {'Mix-F1':>8}"
    sep = "-" * len(header)
    print(sep)
    print(header)
    print(sep)
    for name, m in results:
        pc = m["per_class"]
        print(f"{name:<20} {m['macro_f1']:>10.4f} {m['accuracy']:>10.4f} "
              f"{pc['positive']['f1']:>8.4f} {pc['negative']['f1']:>8.4f} "
              f"{pc['neutral']['f1']:>8.4f} {pc['mixed']['f1']:>8.4f}")
    print(sep)

    # Highlight best
    best_name, best_m = max(results, key=lambda r: r[1]["macro_f1"])
    print(f"\nBest baseline: {best_name} (macro_f1 = {best_m['macro_f1']:.4f})")
