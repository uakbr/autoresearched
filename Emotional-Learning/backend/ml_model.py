...
def train_ml_model(
    ...
    Steps:
      1. Convert the texts into numeric vectors using TfidfVectorizer.
    ...
    vectorizer = TfidfVectorizer(max_features=500, stop_words="english", ngram_range=(1, 2), sublinear_tf=True)
    X = vectorizer.fit_transform(texts)

    model = LogisticRegression(max_iter=1000, warm_start=True, random_state=42)
    model.fit(X, labels)
...
class MLMoodAnalyzer:
    def __init__(self, texts: List[str] = None, labels: List[str] = None) -> None:
        texts = texts if texts is not None else SAMPLE_POSTS
        labels = labels if labels is not None else TRUE_LABELS
        self.vectorizer, self.model = train_ml_model(texts, labels)

    def analyze(self, text: str) -> dict:
        label = predict_single_text(text, self.vectorizer, self.model)
